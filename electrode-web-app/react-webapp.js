  ...
  return (options) => {
    ...
    const makeHeaderBundles = () => {
      ...
      const manifestLink = manifest
        ...
        : "";
      const css = bundleCss();
      const enterHead = routeOptions.unbundledJS.enterHead;
      const unbundledStyle = routeOptions.unbundledStyle;
      let jsLink = "";
      let unbundledCssLink = "";
      if (enterHead.length > 0) {
        for (let i = 0; i < enterHead.length; i++) {
          const src = enterHead[i].src;
          jsLink += `<script src="${src}"></script>`;
        }
      }
      if (unbundledStyle.length > 0) {
        for (let i = 0; i < unbundledStyle.length; i++) {
          const href = unbundledStyle[i];
          unbundledCssLink += `<link rel="stylesheet" href="${href}" />`;
        }
      }
      const cssLink = css && !criticalCSS
        ? `<link rel="stylesheet" href="${css}" />`
        : "";
      return `${manifestLink}${jsLink}${cssLink}${unbundledCssLink}`;
    };
